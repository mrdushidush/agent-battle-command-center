...
/**
 * Ollama Battle Test - Creates all 5 tasks upfront (visible as buildings),
 * then the agent destroys them one by one.
 */
...
async function resetSystem() {
  ...
  try {
    const r = await fetch(`${API_BASE}/tasks`, { headers: { 'X-API-Key': API_KEY } });
    const existing = await r.json();
    for (const t of existing) {
      if (['pending', 'assigned', 'in_progress'].includes(t.status)) {
        await fetch(`${API_BASE}/tasks/${t.id}`, { method: 'DELETE', headers: { 'X-API-Key': API_KEY } });
      }
    }
    console.log('   ✓ Pending tasks cleared');
  } catch (e) {
    console.log('   ⚠ Could not clear tasks');
  }
  await sleep(2000);
}
...
DO NOT just output the code - you MUST call file_write(path="tasks/${fileName}.py", content="...")`;
  ...
  const response = await fetch(`${API_BASE}/tasks`, {
    ...
    body: JSON.stringify({
      title: `[BATTLE-C${task.complexity}] ${task.name}`,
...
async function main() {
  ...
  console.log('⚔️  OLLAMA BATTLE TEST - 5 TARGETS');
  console.log('═'.repeat(50));
  console.log('All 5 targets spawn first, then agent destroys them\n');

  await resetSystem();

  const startTime = Date.now();

  // ── Phase 1: Create all 5 tasks (buildings appear on battlefield) ──
  console.log('📦 PHASE 1: Deploying targets...\n');
  const createdTasks = [];
  for (let i = 0; i < TASKS.length; i++) {
    const task = TASKS[i];
    const created = await createTask(task);
    createdTasks.push({ ...task, id: created.id });
    console.log(`   🏗️  [${i + 1}/5] C${task.complexity} ${task.name} → ${created.id.substring(0, 8)}...`);
  }

  console.log('\n   ✅ All 5 targets deployed! Watch the battlefield...');
  await sleep(3000); // Let user see all 5 buildings

  // ── Phase 2: Destroy targets one by one ──
  console.log('\n🎯 PHASE 2: Engaging targets...\n');
  let passed = 0;

  for (let i = 0; i < createdTasks.length; i++) {
    const task = createdTasks[i];
    console.log(`[${i + 1}/5] ⚔️  C${task.complexity} ${task.name}`);
    ...
    try {
      ...
      const execResponse = await executeTask(task.id, task.description);
      ...
      if (await runValidation(task.validation)) {
        passed++;
        console.log(`   ✅ TARGET DESTROYED (${duration}s)`);
      } else {
        console.log(`   ❌ TARGET SURVIVED (${duration}s)`);
      }
  ...
  console.log('\n' + '═'.repeat(50));
  console.log(`📊 BATTLE RESULT: ${passed}/5 destroyed (${Math.round(passed/5*100)}%) in ${Math.floor(totalDuration/60)}m ${totalDuration%60}s`);
  console.log('═'.repeat(50));
}
