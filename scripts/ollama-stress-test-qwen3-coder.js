 ...
 *
 * Tests qwen3-coder:16k (MoE, 30.5B total, 3.3B active) against the same
 * 20-task benchmark that qwen2.5-coder:7b achieves 100% on.
 *
 ...
 *
 * Run with: node scripts/ollama-stress-test-qwen3-coder.js
 */

const API_BASE = 'http://localhost:3001/api';
const AGENTS_BASE = 'http://localhost:8000';
const API_KEY = process.env.API_KEY || 'ceb3e905f7b1b5e899645c6ec467ca34';
const TASK_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes per task (MoE model is slower)
const REST_DELAY_MS = 5000; // 5 seconds rest (model needs more breathing room)
const RESET_EVERY_N_TASKS = 3; // Reset agent memory every 3 tasks
const MODEL_NAME = 'qwen3-coder:16k'; // Custom Modelfile with temp=0

async function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

// Same 20-task suite as other comparison tests
const TASKS = [
  // ═══════════════════════════════════════════════════════════════════════════
...
DO NOT just output the code - you MUST call file_write(path="tasks/${fileName}.py", content="...")`;
  ...
  const response = await fetch(`${API_BASE}/tasks`, {
    ...
    body: JSON.stringify({
      title: `[QWEN3-CODER-C${task.complexity}] ${task.name}`,
...
async function executeTask(taskId, description) {
  const response = await fetch(`${AGENTS_BASE}/execute`, {
    ...
    body: JSON.stringify({
      ...
      use_claude: false,
      model: `ollama/${MODEL_NAME}`  // Override to use qwen3-coder model
    })
  });
...
async function main() {
  ...
  console.log('\u{1f525} OLLAMA STRESS TEST - QWEN3-CODER:30B (MoE) COMPARISON');
  console.log('\u2550'.repeat(70));
  console.log(`Model: ${MODEL_NAME} (30.5B total, 3.3B active per token)`);
  console.log('Baseline: qwen2.5-coder:7b = 100% (20 tasks C1-C8)');
  console.log(`Tasks: ${TASKS.length} | Complexity range: 1-8`);
  console.log(`Rest: ${REST_DELAY_MS/1000}s between tasks | Reset every ${RESET_EVERY_N_TASKS} tasks`);
  console.log(`Task timeout: ${TASK_TIMEOUT_MS/1000}s`);
  console.log('\u2550'.repeat(70) + '\n');
  ...
  for (let i = 0; i < TASKS.length; i++) {
    ...
    try {
      ...
      console.log(`   Executing with ${MODEL_NAME}...`);
      const execPromise = executeTask(created.id, task.description);
  ...
  console.log('\n' + '\u2550'.repeat(70));
  console.log('\u{1f4ca} QWEN3-CODER:30B STRESS TEST RESULTS');
  console.log('\u2550'.repeat(70));
  console.log(`   Model:    ${MODEL_NAME} (MoE 30.5B/3.3B active)`);
  ...
  console.log('\n\u{1f50d} COMPARISON vs qwen2.5-coder:7b (baseline):');
  console.log('\u2500'.repeat(50));
  console.log(`   qwen2.5-coder:7b  : 100% (20/20) ~12s avg`);
  console.log(`   qwen3-coder:30b   : ${successRate}% (${results.passed}/${results.total}) ~${Math.round(totalDuration / TASKS.length)}s avg`);

  if (successRate > 95) {
    console.log('\n\u{1f3af} VERDICT: qwen3-coder:30b matches baseline - viable if speed is acceptable');
  } else if (successRate >= 80) {
    console.log('\n\u{1f3af} VERDICT: qwen3-coder:30b close but not quite - may need tuning');
  } else {
    console.log('\n\u{1f3af} VERDICT: qwen3-coder:30b underperforms on 8GB VRAM');
  }

  if (foundThreshold) {
    console.log(`   Breaks down at C${foundThreshold}+`);
  }
  ...
  const fs = require('fs');
  fs.writeFileSync(
    'scripts/ollama-stress-results-qwen3-coder.json',
    JSON.stringify({
      ...results,
      totalDuration,
      successRate,
      baseline: { model: 'qwen2.5-coder:7b', successRate: 100, avgTime: 12 },
      recommendedThreshold: foundThreshold ? foundThreshold - 1 : 8
    }, null, 2)
  );
  console.log('\n\u{1f4be} Results saved to scripts/ollama-stress-results-qwen3-coder.json');
